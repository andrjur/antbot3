...
### Бэкапы и импорт
```
/export_db — экспорт базы в JSON (сохраняет в backups/)
/import_db — полный импорт базы (⚠️ ОПАСНО: удаляет всех студентов!)
/import_db_safe — безопасный импорт (восстанавливает курсы/уроки, сохраняет студентов)
```
...
```bash
# Просмотр всех выполненных ДЗ
/view_homework

# Просмотр ДЗ конкретного курса
/view_homework spring3

# Экспорт базы (бот отправит файл + сохранит в backups/)
/export_db

# Безопасное восстановление курсов из бэкапа
/import_db_safe  # Затем отправить JSON-файл боту
```
