...
### ⚠️ КРИТИЧНО: Закончилось место на диске

**Симптомы:** Бот не запускается, n8n не отвечает (ошибка 521), логи не пишутся.

**Проверка:**
```bash
df -h
# Если Use% = 100% для / — диск полон!
```

**Решение:**
```bash
# 1. Очистить Docker (самое действенное)
...
**Причина:** Docker образы и кэш занимают много места. Делайте очистку регулярно.

---

### n8n: Ошибка 521 Cloudflare (Web server is down)

**Симптомы:** n8n работает (`docker-compose ps n8n` — Up), но `https://n8n.indikov.ru` выдаёт ошибку 521.

**Причина:** Cloudflare стучится на порт 443, а n8n слушает порт 5678.

**Решение — создать Origin Rule в Cloudflare:**

1. Зайдите в **Cloudflare Dashboard** → **Rules** → **Origin Rules**
2. Нажмите **Create Rule**
3. Настройте:
   - **Field:** Hostname
   - **Operator:** equals
...
### 1. settings.json перезаписывается при git pull

**Проблема:** settings.json содержит настройки курсов и не должен попадать в git.

**Решение:** Файл уже добавлен в .gitignore. Если случайно закоммитили:
```bash
git rm --cached settings.json
git commit -m "Remove settings.json from tracking"
```

### 2. Docker создаёт settings.json как директорию

**Проблема:** Если settings.json не существует, Docker создаёт его как директорию, что ломает бот.

**Решение:** Перед первым запуском:
```bash
echo '{}' > settings.json
```

### 3. pending_admin_homework использует created_at, не timestamp

**Проблема:** В таблице pending_admin_homework используется колонка `created_at` (TIMESTAMP DEFAULT CURRENT_TIMESTAMP), а не `timestamp`.

**Важно:** При проверке таймаута ДЗ используйте формат 'YYYY-MM-DD HH:MM:SS', не ISO format:
```python
cutoff_time_str = cutoff_time.strftime('%Y-%m-%d %H:%M:%S')
# НЕ: cutoff_time.isoformat()
```

### 4. MarkdownV2 звёзды (*) показываются буквально

**Проблема:** В Telegram при parse_mode=MarkdownV2 звёзды `*текст*` показываются как звёзды, а не как жирный текст.

**Решение:** Используйте `parse_mode=None` и экранирование `escape_md()`:
```python
await message.answer(text, parse_mode=None)
```

### 5. n8n права доступа (Permission denied)

**Проблема:** n8n падает с `EACCES: permission denied`.
...
**Проблема:** Если в .env есть дубликаты переменных (например, BOT_TOKEN дважды), используется последнее значение.

**Решение:** Проверьте .env на дубликаты перед деплоем.

### 7. WEBAPP_HOST=0.0.0.0 обязателен

**Проблема:** Без этой переменной aiohttp может слушать только IPv6 (`::1`).
...
### 8. Нельзя менять схему БД в коде

**Правило:** Никогда не используйте ALTER TABLE в коде бота. Используйте существующие колонки.

### 9. Админ-система: суперадмины vs админы группы

- **Суперадмины** — из .env ADMIN_IDS (можно экспортировать/импортировать БД)
- **Админы группы** — участники ADMIN_GROUP_ID (могут управлять курсами)
- **Любой админ** может добавить другого админа через `/add_admin`

### 10. 2-минутный таймаут ДЗ

После 2 минут ожидания ДЗ отправляется на n8n webhook. Админ имеет приоритет:
- Если админ ответил ДО ответа от n8n — решение админа приоритетно
- Если n8n ответил первым — результат сохраняется, но админ может перезаписать

---

## 🛠 Устранение неполадок (Troubleshooting)

### 1. n8n или Бот не открываются (Ошибка 521 Web server is down)

Cloudflare видит сервер, но сервер отвергает соединение.

**Решение:**

Проверьте порты в Firewall (UFW):
```bash
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp
sudo ufw allow 5678/tcp  # Для n8n
sudo ufw reload
```

Проверьте правила Cloudflare (Origin Rules):
- Перейдите в Rules → Origin Rules
- Создайте правило для `n8n.your-domain.com`
- Destination Port → Rewrite to `5678`

### 2. Ошибка 525 SSL Handshake Failed

Контейнеры внутри работают по HTTP, Cloudflare не может установить защищенное соединение.

**Решение:**
- В Cloudflare перейдите в SSL/TLS → Overview
- Установите режим: **Flexible** (Гибкий)

### 3. Ошибка базы данных n8n (SQLITE_ERROR: no such column)

Происходит, если обновление n8n прервалось (например, из-за нехватки места).

**Решение (полный сброс базы n8n):**

⚠️ Внимание: это удалит все воркфлоу n8n!

```bash
# 1. Остановить контейнеры
docker-compose down

# 2. Удалить поврежденные файлы базы
sudo rm -rf n8n_data/*
...
Если сервер пишет `OSError: [Errno 28] No space left on device`:

```bash
# 1. Проверить свободное место
df -h

# 2. Удалить неиспользуемые образы Docker, кэш и контейнеры
docker system prune -af

# 3. Очистить системные логи
sudo journalctl --vacuum-time=1d

# 4. Очистить логи бота
rm -f logs/*.log
```

### 🐌 Проблемы с производительностью (Зависания)

Если при обновлении сервер перестает отвечать, а CPU = 100% — не хватает RAM.

**Решение: Добавить Swap (файл подкачки):**
...
# 5. Оптимизировать (использовать RAM по максимуму)
sudo sysctl vm.swappiness=10
echo 'vm.swappiness=10' | sudo tee -a /etc/sysctl.conf
```

### Права доступа к папкам (Permission Denied)

```bash
# Выдать права для n8n
sudo chown -R 1000:1000 n8n_data
sudo chmod -R 777 n8n_data
```
...
# 4. Очистить старый образ (освобождает 1-2 ГБ)
docker image prune -f
```

### Импорт workflow в n8n

Workflow для AI-проверки ДЗ находится в папке `n8n_workflows/AI_Agent.json`.
