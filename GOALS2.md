...
### Fix 12: Финальная сборка для n8n 2.8.3 (03.03.2026)

**Проблемы:**

1. **`"options": null`** — валидатор n8n 2.8.3 падает при импорте
2. **`$input.first()[0]`** — опечатка в Process Video (лишнее `[0]`)
3. **`.item.json`** — устаревший синтаксис в Prepare Text Messages

**Решение:**

1. **Удалены все `options: null` и `options: {}`:**
   - Webhook-homework
   - Edit Fields
   - If
   - HTTP Request

2. **Исправлен Process Video:**
   ```javascript
   // БЫЛО:
   $input.first()[0].binary?.data  // ← [0] ломает!
...
3. **Исправлен Prepare Text Messages:**
   ```javascript
   // БЫЛО:
   $input.item.json.hw_text
   $('Edit Fields').item.json.student_name
   
   // СТАЛО:
   $input.first().json.hw_text
   $('Edit Fields').first().json.student_name
   ```

**Результат:**
- ✅ JSON валиден для n8n 2.8.3
- ✅ Импорт работает без ошибок
- ✅ Современный синтаксис Code нод

---

### Fix 13: Switch typeVersion 3 → 2 для совместимости (03.03.2026)
...
**Причина:**
Switch нода с `typeVersion: 3` не совместима со старыми версиями n8n (включая 2.8.3).

**Что было:**
```json
{
  "type": "n8n-nodes-base.switch",
  "typeVersion": 3  // ← НЕ СОВМЕСТИМО!
}
```

**Решение:**
Заменили версию на 2:
```json
{
  "type": "n8n-nodes-base.switch",
  "typeVersion": 2  // ← СОВМЕСТИМО!
}
```

**Важно:**
- Switch v3 работает только в n8n 2.9+
- Switch v2 работает во всех версиях включая 2.8.3
- Функционал одинаковый (маршрутизация по mimeType)

**Результат:**
- ✅ Импорт работает на n8n 2.8.3
- ✅ Маршрутизация работает корректно
- ✅ Нет ошибок `Could not find property option`

---

### Fix 14: If нода — добавлен options block (03.03.2026)

**Проблема:**
If нода падала с ошибкой `Could not find property option` на n8n 2.8.3.

**Причина:**
У If ноды версии 2.2 отсутствует блок `options` внутри `conditions`, из-за чего валидатор n8n не может прочитать `caseSensitive`.
...
**Решение:**
Добавлен блок `options` внутрь `conditions`:
...
**Результат:**
- ✅ If нода работает на n8n 2.8.3
- ✅ Нет ошибок валидации
- ✅ Проверка hw_file_id работает корректно

---

### Fix 15: Switch expression исправлено (03.03.2026)

**Проблема:**
Switch нода некорректно маршрутизировала файлы.

**Причина:**
Выражение для проверки mimeType было сложным и могло не работать.

**Решение:**
Упрощено выражение до прямой проверки `startsWith`:
...
**Результат:**
- ✅ Switch корректно маршрутизирует по типу файла
- ✅ Фото → Process Photo
- ✅ Аудио → Process Audio
- ✅ Видео → Process Video
...
## ✅ ИТОГОВЫЙ ЧЕК-ЛИСТ ДЛЯ ИМПОРТА В n8n 2.8.3:

### 1. Подготовка файла:

- ✅ Switch typeVersion: 2 (не 3!)
- ✅ If нода: options в conditions
- ✅ Нет options: null
- ✅ Нет options: {}
- ✅ .first().json (не .item.json)

### 2. Импорт:

- ✅ Workflows → Import From File
- ✅ Выбрать n8n_flow.json
- ✅ Без ошибок импорта

### 3. Очистка:

- ✅ Нажать Корзину 🗑️ (Clear run data)
- ✅ Иначе ноды будут зависать
...
- ✅ Отправить ДЗ с фото → Process Photo
- ✅ Отправить ДЗ с голосовым → Process Audio
- ✅ Отправить ДЗ с кружочком → Process Video
...
### Fix 17: Очистка ввода от кавычек и мусора при создании курса (03.03.2026)

**Проблема:**
Админ ввёл ID группы с кавычками и двоеточием: `-1003820533058":`
В результате:
- В БД записалось: `group_id='-1003820533058":'`
- settings.json повреждён: `Expecting property name enclosed in double quotes`

**Причина:**
Не было очистки ввода от специальных символов.

**Решение:**
Добавлена очистка для всех полей при создании курса:

**1. Очистка group_id:**
```python
raw_group_id = message.text.strip()
# Удаляем кавычки, двоеточия, backticks
raw_group_id = raw_group_id.replace('"', '').replace("'", '').replace(':', '').replace('`', '').strip()

# Авто-добавление минуса если нет
if not raw_group_id.startswith("-") and raw_group_id.isdigit():
...
# В БД и settings.json записывается корректно!
```

**Важно:**
- Если ID введён без минуса (`1003820533058`) — автоматически добавляется `-`
- Кавычки, двоеточия, backticks — удаляются
- Проверка валидности: ID должен быть целым числом
...
#### 8. JSON parameter needs to be valid JSON — пра��ильный JavaScript-объект
**Проблема:**
```javascript
={
  "messages": [
