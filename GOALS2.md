...
### Fix 2: Конфликт обработчиков (aiogram 3 Routing)
...
### Fix 3: Защита от дурака в `/add_course`
...
### Fix 4: Ошибка БД при ручной загрузке контента (`NOT NULL constraint failed`)
...
### Fix 5: Ошибка `charset` в aiohttp (Мониторинг Prometheus)
