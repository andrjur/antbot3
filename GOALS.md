...
### 4. hw_status='approved' сразу после активации
**Проблема:** 
```
20:05:47 [send_main_menu] - homework_pending=True ✅
20:05:55 [handle_homework] - hw_status='approved' ❌ ЧЕРЕЗ 8 СЕКУНД!
```

**Гипотезы:**
1. **Гонка данных (race condition)** — два процесса обращаются к БД
2. **Фоновая задача** — планировщик или триггер одобряет ДЗ
3. **БД по умолчанию** — hw_status='approved' вместо NULL
4. **Другой обработчик** — что-то ещё меняет статус

**Диагностика:**
- Добавлено логирование ПЕРЕД проверкой hw_status
- Добавлено логирование ПОСЛЕ UPDATE в `_update_user_course_after_lesson`
- Проверка что записалось в БД

**Структура проблемы:**
```
1. Активация курса → hw_status='none' (из activate_course)
2. Отправка урока 1 → hw_status='pending' (из _update_user_course_after_lesson)
3. ??? → hw_status='approved' (НЕИЗВЕСТНО ГДЕ!)
4. Проверка ДЗ → "ДЗ уже засчитано"
```

**Решение в процессе:**
- Детальное логирование всех изменений hw_status
- Проверка БД после каждого UPDATE
- Поиск фоновых задач которые меняют hw_status
